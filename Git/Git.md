...
※ファイルの削除をコミットする場合は、`git add <file>`でなく`git rm <file>`を使用する。
...
ローカルをリモートリポジトリへ強制的にあわせる
----------------------------------------------
以下の手順を実施すると、ローカルの変更がまっさらになった状態でしきりなおせる。
...
フェッチ
========
リモートリポジトリの変更分をローカルのトラッキングブランチへ反映する。
ローカルのファイルに反映されるわけではない。
(例えば、masterブランチを使っているのであれば、origin/masterが更新される)
...
マージ
======
カレントのブランチに別のブランチを結合する。
リモートのmasterブランチの変更分をローカルのファイルに反映するには、以下の手順を行う。
